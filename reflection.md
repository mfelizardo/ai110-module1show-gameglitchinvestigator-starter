...
- Give one example of an AI suggestion that was incorrect or misleading (including what the AI suggested and how you verified the result).
  ...
  - I asked the AI to generate tests to verify the fix to the New Game button starting a new game after winning or losing the previous game was properly implemented. It then told me that app.py couldn't be imported directly and that it would instead create mirrored functions that copy the same logic from the app and test those instead. I pushed back, stating that those tests would be pointless as it wasn't actually testing the code in `app.py`, and we eventually reached a resolution of using Streamlit's AppTest utility in order to properly test the app.
